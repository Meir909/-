...
module.exports = async (req, res) => {
  ...
  try {
    if (req.method === 'GET') {
      ...
      return res.json({ ok: true, images: imgs });
    }

    if (req.method === 'POST') {
      // Add photo: { name, base64 }
    ...
    if (req.method === 'DELETE') {
      // Delete photo: { ghPath, replaceSrc, replacePath }
      ...
      // Update gallery.html: replace deleted block src with replaceSrc, remove replaceSrc block
      const galData = await ghGet('public/gallery.html');
      ...
      if (delBlock && replBlock) {
        const escapedDel = delName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const newDelBlock = delBlock
          .replace(new RegExp(`src="images/${escapedDel}"`), `src="${replaceSrc}"`)
